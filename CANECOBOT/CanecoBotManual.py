...
# MANUAL PROCESS SUR VERSION 2022
...
folderpath = r"C:\Users\yabde\Documents\GitHub\sphere_bot\CANECOBOT"
targetpath = os.path.join(folderpath,"CENTRIMEX.afr")
...
mapping_folder = os.path.join(folderpath,"mapping.xlsx")
mapFile = pd.read_excel(mapping_folder)
...
coldefault_folder = os.path.join(folderpath,"ColDefaut.xlsx")
# reading ColDefaut excel file for the filling of the import frame        
defaultFile = pd.read_excel(coldefault_folder)
...
destFolder = r'C:\ProgramData\ALPI\Caneco BT\5.8\Labels'   # this is the only folder where Caneco can pick up the logos 
...
def one_application_startup():

    "Only if caneco is close"
    caneco = Application(backend='uia')
    caneco.start(r"C:\Program Files (x86)\ALPI\Caneco BT\5.8\Caneco5.exe")
...
def two_canneco_already_opened():

    #Reconnecting to Caneco when already opened
    caneco = Application(backend='uia')
    caneco.connect(path=r"C:\Program Files (x86)\ALPI\Caneco BT\5.8\Caneco5.exe")
    win = caneco.top_window()
    print("--RECONNECTED TO CANECO--")

    winFull(win) # checking for fullscreen 

def three_impport_data():
...
def four_export_from_csv_to_caneco():
...
# one_application_startup()
two_canneco_already_opened()
four_export_from_csv_to_caneco()